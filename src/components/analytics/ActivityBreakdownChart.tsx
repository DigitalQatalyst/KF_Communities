...
export function ActivityBreakdownChart({
  ...
  const fetchActivityData = async () => {
    ...
    const activityData: ActivityData[] = await Promise.all(communities.map(async community => {
      const [postsResult, commentsResult] = await Promise.all([safeFetch(supabase.from('posts').select('id', {
        count: 'exact',
        head: true
      }).eq('community_id', community.id).gte('created_at', thirtyDaysAgo.toISOString()).eq('status', 'active')), safeFetch(supabase.from('comments').select('id, post_id').gte('created_at', thirtyDaysAgo.toISOString()).eq('status', 'active'))]);
      let commentsCount = 0;
      if (commentsResult[0]) {
        // Get post IDs for this community
        const postsInCommunityQuery = supabase.from('posts').select('id').eq('community_id', community.id);
        const [postsInCommunity] = await safeFetch(postsInCommunityQuery);
        if (postsInCommunity) {
          const postIds = new Set(postsInCommunity.map((p: any) => p.id));
          commentsCount = commentsResult[0].filter((c: any) => postIds.has(c.post_id)).length;
        }
      }
      return {
        name: community.name.length > 15 ? community.name.substring(0, 15) + '...' : community.name,
  ...
  return <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden animate-fade-in">
      ...
      <div className="p-6">
        {data.length === 0 ? <div className="border border-dashed border-gray-300 rounded-lg p-6 text-center">
