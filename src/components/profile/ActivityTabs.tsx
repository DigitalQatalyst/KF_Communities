      ...
      <Tabs defaultValue="posts" className="w-full">
        ...
        <TabsContent value="posts" className="p-6 m-0">
          {postsLoading ? <div className="space-y-4">
            ...
            </div> : postsError ? <div className="border border-yellow-200 bg-yellow-50 text-yellow-800 p-3 rounded-md text-sm flex items-center justify-between">
            ...
            </div> : posts.length === 0 ? <div className="border border-dashed border-gray-300 rounded-lg p-6 text-center">
              <FileText className="h-8 w-8 text-gray-400 mx-auto mb-2" />
              <p className="text-sm text-gray-500">No posts yet</p>
            </div> : <div className="space-y-4">
              {posts.map(post => <Link key={post.id} to={`/post/${post.id}`} className="block border border-gray-100 rounded-lg p-4 hover:border-gray-200 hover:shadow-sm transition-all hover-scale">
        ...
        <TabsContent value="comments" className="p-6 m-0">
          {commentsLoading ? <div className="space-y-4">
              {[1, 2, 3].map(i => <Skeleton key={i} className="h-24 w-full" />)}
            </div> : commentsError ? <div className="border border-yellow-200 bg-yellow-50 text-yellow-800 p-3 rounded-md text-sm flex items-center justify-between">
              <div className="flex items-center gap-2">
                <AlertCircle className="h-4 w-4" />
            ...
            </div> : comments.length === 0 ? <div className="border border-dashed border-gray-300 rounded-lg p-6 text-center">
              <MessageSquare className="h-8 w-8 text-gray-400 mx-auto mb-2" />
              <p className="text-sm text-gray-500">No comments yet</p>
            </div> : <div className="space-y-4">
              {comments.map(comment => <Link key={comment.id} to={`/post/${comment.post_id}`} className="block border border-gray-100 rounded-lg p-4 hover:border-gray-200 hover:shadow-sm transition-all hover-scale">
                  <div className="flex items-start justify-between gap-3 mb-2">
