...
export function NewConversationModal({
  ...
  const searchUsers = useCallback(async (query: string) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.rpc('search_users', {
        query,
  ...
  const createDirectConversation = async (otherUserId: string) => {
    ...
    try {
      ...
      } = await supabase.from('conversation_participants').select('conversation_id').eq('user_id', currentUserId);
      if (myConversations && myConversations.length > 0) {
        const conversationIds = myConversations.map(c => c.conversation_id);
        const {
          data: existingConvo
        } = await supabase.from('conversation_participants').select('conversation_id, conversations!inner(type)').eq('user_id', otherUserId).in('conversation_id', conversationIds);
        const directConvo = existingConvo?.find((c: any) => c.conversations.type === 'direct');
        if (directConvo) {
        ...
        <Tabs defaultValue="direct" className="w-full">
          ...
          <TabsContent value="direct" className="mt-4">
            <ScrollArea className="h-[300px]">
              {searching ? <div className="text-center py-8">
                  <div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground">Searching...</p>
                </div> : users.length === 0 && searchQuery ? <div className="text-center py-8">
                  ...
                  {users.map(user => <button key={user.id} onClick={() => createDirectConversation(user.id)} disabled={loading} className="w-full p-3 rounded-lg flex items-center gap-3 hover:bg-accent transition-colors disabled:opacity-50">
