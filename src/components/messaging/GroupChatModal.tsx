...
export function GroupChatModal({
  ...
  const fetchCommunities = async () => {
    try {
      ...
      const {
        data: memberships
      } = await supabase.from('memberships').select('community_id').eq('user_id', currentUserId);
      ...
      const {
        data,
        error
      } = await supabase.from('communities').select('id, name, description').in('id', communityIds).order('name');
  ...
  const fetchCommunityMembers = async (communityId: string) => {
    try {
      ...
      } = await supabase.from('memberships').select('user_id').eq('community_id', communityId);
      ...
      const {
        data: users,
        error: usersError
      } = await supabase.from('users_local').select('id, username, avatar_url, email').in('id', userIds);
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          {step === 1 && <ScrollArea className="h-[300px]">
              {communities.length === 0 ? <div className="text-center py-8">
                  <Users className="h-12 w-12 text-muted-foreground mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground">No communities found</p>
                </div> : <div className="space-y-2">
                  {communities.map(community => <button key={community.id} onClick={() => setSelectedCommunity(community.id)} className={`w-full p-3 rounded-lg flex items-center gap-3 transition-colors ${selectedCommunity === community.id ? 'bg-primary text-primary-foreground' : 'hover:bg-accent'}`}>
                      <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                        <Users className="h-5 w-5" />
                      </div>
                      <div className="flex-1 text-left">
                        <p className="text-sm font-medium">{community.name}</p>
                        {community.description && <p className="text-xs opacity-80 truncate">{community.description}</p>}
                      </div>
                    </button>)}
                </div>}
            </ScrollArea>}
          ...
          {step === 2 && <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="groupName" placeholder="Enter group name..." value={groupName} onChange={e => setGroupName(e.target.value)} autoFocus />
              </div>
            </div>}

          {step === 3 && <ScrollArea className="h-[300px]">
              {members.length === 0 ? <div className="text-center py-8">
                  <User className="h-12 w-12 text-muted-foreground mx-auto mb-2" />
                  <p className="text-sm text-muted-foreground">No members found</p>
                </div> : <div className="space-y-2">
                  {members.filter(m => m.user_id !== currentUserId).map(member => <div key={member.user_id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent transition-colors">
          ...
          <div className="flex gap-2 justify-end">
            ...
            {step < 3 ? <Button onClick={nextStep}>Next</Button> : <Button onClick={createGroupChat} disabled={loading}>
