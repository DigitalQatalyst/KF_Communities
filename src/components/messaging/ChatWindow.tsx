...
export function ChatWindow({
  ...
  const fetchConversationInfo = async () => {
    try {
      ...
      } = await supabase.from('conversation_participants').select('role').eq('conversation_id', conversationId).eq('user_id', user.id).single();
      if (participantData) {
  ...
  const fetchSenderInfo = async (message: Message) => {
    const {
      data: sender
    } = await supabase.from('users_local').select('id, username, avatar_url').eq('id', message.sender_id).single();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: participants
      } = await supabase.from('conversation_participants').select('user_id').eq('conversation_id', conversationId).neq('user_id', currentUserId);
      if (participants) {
        const notificationPromises = participants.map(p => supabase.from('notifications').insert({
          user_id: p.user_id,
          type: 'comment',
          title: 'New message',
  ...
  if (loading) {
    return <div className="flex flex-col h-full">
        ...
        <div className="flex-1 p-4 space-y-4">
          {[1, 2, 3].map(i => <div key={i} className="flex items-start gap-3">
  ...
  return <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-foreground flex items-center gap-2">
              {conversationInfo?.type === 'direct' ? conversationInfo.other_user?.username || 'Unknown User' : conversationInfo?.name || 'Group Chat'}
              {conversationInfo?.type === 'group' && conversationInfo?.community_name && <Badge variant="secondary" className="text-xs">
                  {conversationInfo.community_name}
                </Badge>}
            </h3>
            {conversationInfo?.type === 'group' && <p className="text-xs text-muted-foreground">
                {conversationInfo.participants?.length || 0} members
              </p>}
          </div>
        </div>

        {conversationInfo?.type === 'group' && <div className="flex items-center gap-2">
            {canManageGroup && <Button variant="outline" size="sm" onClick={() => setIsAddMemberOpen(true)} className="gap-2">
            ...
            <Sheet>
              ...
              <SheetContent>
                ...
                <ParticipantList conversationId={conversationId} currentUserId={currentUserId || ''} currentUserRole={currentUserRole} onParticipantRemoved={fetchConversationInfo} />
              </SheetContent>
            </Sheet>

            {currentUserRole !== 'owner' ? <AlertDialog>
      ...
      <ScrollArea className="flex-1 p-4" ref={scrollAreaRef}>
        {messages.length === 0 ? <div className="h-full flex items-center justify-center text-center p-8">
            <div>
            ...
            {messages.map(message => <MessageBubble key={message.id} message={message} isOwnMessage={message.sender_id === currentUserId} />)}
          </div>}
      </ScrollArea>

      <div className="border-t border-border p-4">
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          <Input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder={isOnline ? "Type a message..." : "Offline - messages will send when online"} disabled={!isOnline || sending} className="flex-1" />
          <Button type="submit" size="icon" disabled={!newMessage.trim() || !isOnline || sending}>
            <Send className="h-4 w-4" />
          </Button>
        </form>
      </div>

      {conversationInfo?.type === 'group' && <AddMemberModal open={isAddMemberOpen} onOpenChange={setIsAddMemberOpen} conversationId={conversationId} communityId={conversationInfo.community_id} />}
    </div>;
}