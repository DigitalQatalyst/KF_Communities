...
export function AddMemberModal({
  ...
  const fetchAvailableMembers = async () => {
    ...
    try {
      ...
      } = await supabase.from('conversation_participants').select('user_id').eq('conversation_id', conversationId).is('left_at', null);
      ...
      const {
        data: users
      } = await supabase.from('users_local').select('id, username, avatar_url, email').in('id', availableUserIds);
  ...
  const filteredMembers = availableMembers.filter(m => m.username?.toLowerCase().includes(searchQuery.toLowerCase()) || m.email.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          ...
          <ScrollArea className="h-[300px]">
            {filteredMembers.length === 0 ? <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  {availableMembers.length === 0 ? 'All community members are already in this chat' : 'No members found'}
                </p>
              </div> : <div className="space-y-2">
                {filteredMembers.map(member => <div key={member.user_id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-accent transition-colors">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button onClick={addMembers} disabled={loading || selectedMembers.size === 0}>
